...
Функция ИдентификаторПравил(Знач пДанные)
	...
	Если ЗначениеЗаполнено(пДанные.Правило) Тогда
		...
		// если идентификатор правила определен плагинами edt и не переопределен по словарю,
		// то сократим его до смысловой части
		мЧастиИдентификатора = СтрРазделить(пДанные.Правило, ":", Ложь);
		Если мЧастиИдентификатора.Количество() = 2
			И СтрНачинаетсяС(мЧастиИдентификатора[0], "com.e1c") Тогда

			Возврат мЧастиИдентификатора[1];
