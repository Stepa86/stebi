...
Функция АвтоСиноним(НовыеСинонимы, Знач Правило, Знач ДанныеПереопределения)
	...
	Для каждого КЗ Из ИсточникПравил Цикл
		...
		Если Не СтрНачинаетсяС(Стр.Правило, КЗ.Ключ) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ПустаяСтрока(КЗ.Значение) Тогда
			новыйСиноним = СтрЗаменить(Стр.Правило, КЗ.Ключ + ".", "");
		Иначе
			новыйСиноним = СтрЗаменить(Стр.Правило, КЗ.Ключ, КЗ.Значение);
		КонецЕсли;
