...
Процедура ПереопределитьИдентификаторыПравил(таблицаРезультатов)
	...
	Файл = Новый Файл(ОбъединитьПути(ТекущийСценарий().Каталог, "../dict/ruleID_dict.json"));
	...
	Для каждого КЗсловарь Из СловарьПравил Цикл
		Для каждого КЗисточник Из ИсточникПравил Цикл
			Если СтрНачинаетсяС(КЗсловарь.Ключ, КЗисточник.Ключ) Тогда
	...
	ОбработанныеПравила = Новый Соответствие();
	...
	Для каждого Стр Из таблицаРезультатов Цикл
		...
		Иначе
			...
			Если ИмяПравила = Неопределено Тогда
				Если ОбработанныеПравила[Стр.Правило] = Неопределено Тогда
					// если правило определено в rulespaces - значит нужно сообщить,
					// чтобы добавили синоним в словарь
					ОбработанныеПравила[Стр.Правило] = Ложь;
					Для каждого КЗ Из ИсточникПравил Цикл
						Если СтрНачинаетсяС(Стр.Правило, КЗ.Ключ) Тогда
							ОбработанныеПравила[Стр.Правило] = Истина;
							...
							Если ПустаяСтрока(КЗ.Значение) Тогда
								Стр.Правило = СтрЗаменить(Стр.Правило, КЗ.Ключ + ".", "");
							Иначе
								Стр.Правило = СтрЗаменить(Стр.Правило, КЗ.Ключ, КЗ.Значение);
							КонецЕсли;
							Прервать;
						КонецЕсли;
					КонецЦикла;
	...
	Если ОбработанныеПравила.Количество() Тогда
		СписокЗначений = Новый СписокЗначений();
		Для каждого КЗ Из ОбработанныеПравила Цикл
			Если КЗ.Значение Тогда
				СписокЗначений.Добавить(" - " + КЗ.Ключ);		
			КонецЕсли;
		КонецЦикла;
		...
		Для каждого Элем Из СписокЗначений Цикл
			ТекстСообщения = ТекстСообщения + Символы.ПС + Элем.Значение;		
		КонецЦикла;
