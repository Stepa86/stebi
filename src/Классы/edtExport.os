...
Процедура ПереопределитьИдентификаторыПравил(таблицаРезультатов)
	...
	Если ТипЗнч(ИсточникПравил) <> Тип("Массив")
	...
	Для каждого Стр Из таблицаРезультатов Цикл
		...
		Если ПустаяСтрока(Стр.Правило) Тогда
			Продолжить;
		ИначеЕсли ИмяПравила = Неопределено Тогда
			Если ОбработанныеПравила[Стр.Правило] = Неопределено Тогда
				ПрефиксПравил = Лев(Стр.Правило, СтрНайти(Стр.Правило, ":") - 1);
				ПрефиксПравил = Лев(ПрефиксПравил, СтрНайти(Стр.Правило, ".", НаправлениеПоиска.СКонца) - 1);
				Если ИсточникПравил.Найти(ПрефиксПравил) = Неопределено Тогда
