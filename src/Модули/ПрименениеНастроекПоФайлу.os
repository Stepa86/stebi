...
Функция ИсключитьЗамечаниеФильтромПоПодсистеме(Знач пИмяФайла) Экспорт
	...
	// Информация по составу метаданных в подсистеме указывает на описания объектов (xml или mdo),
	// а файлы с замечаниями это bsl файлы. Поэтому сопоставлять можем только каталоги.

	КаталогОбъекта = КаталогФайла(путьКФайлу);

	значениеКеша = _ДанныеФильтраПоПодсистемам.КешФайлов[КаталогОбъекта];
	...
	Если _ДанныеФильтраПоПодсистемам.ЕстьОтбор Тогда
		...
		ВБеломСписке = Не _ДанныеФильтраПоПодсистемам.Отбор.Найти(КаталогОбъекта) = Неопределено;

		ИсключитьФайл = Не ВБеломСписке;

	КонецЕсли;
	
	Если Не ВБеломСписке
		И _ДанныеФильтраПоПодсистемам.ЕстьИсключения Тогда
		
		ВЧерномСписке = Не _ДанныеФильтраПоПодсистемам.Исключения.Найти(КаталогОбъекта) = Неопределено;
		
		ИсключитьФайл = ВЧерномСписке;
		
	КонецЕсли;
	
	_ДанныеФильтраПоПодсистемам.КешФайлов.Вставить(КаталогОбъекта, ИсключитьФайл);

	Возврат ИсключитьФайл;
...
Функция ПодготовитьФильтрПоПодсистемам(Знач пФильтрПоПодсистемам)
	...
	Для Каждого цПодсистема Из СтрРазделить(пФильтрПоПодсистемам, ",") Цикл
		...
		имяПодсистемы = СокрЛП(СтрЗаменить(цПодсистема, """", ""));
...
Функция МассивПутейОбъектов(Знач пСоответствиеСПутями, Знач пГенераторПутей)
	...
	Для Каждого цЭлемент Из пСоответствиеСПутями Цикл
		...
		Если ОбщегоНазначения.ФайлСуществует(путьКОбъекту) Тогда
			
			массивПутей.Добавить(КаталогФайла(путьКОбъекту));
...
Функция КаталогФайла(Знач ПолныйПуть)
	
	// Обрезать строку быстрее и надежнее с точки зрения сохранения слешей.

	ПоследнийСлеш = СтрНайти(ПолныйПуть, "/", НаправлениеПоиска.СКонца);
	Возврат Сред(ПолныйПуть, 1, ПоследнийСлеш);
